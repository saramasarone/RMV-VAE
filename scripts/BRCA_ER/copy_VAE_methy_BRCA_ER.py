...
sns.scatterplot(x=pca_res[:, 0], y=pca_res[:, 1], hue=scaled_methyl.loc[:,'AGR3'], palette='coolwarm', ax = ax3)
...
scores = cross_val_score(clf_embedding, embedding, y, cv=7)
print(np.mean(scores))

# filtered methylation
clf_orig = LogisticRegression(random_state=0, penalty = 'l1', solver = 'saga')
scores_orig = cross_val_score(clf_orig, methy_filtered, y, cv=7)
...
#embedding_df.to_csv('/Users/smasarone/Documents/Omics_datasets_integration/BRCA_experiments/BRCA_embeddings/methy_BRCA_embedding_ER2.csv')
